...
const FeatureCard: React.FC<FeatureCardProps> = ({ title, description, href }) => {
  return (
    <Link
      href={href}
      className="block bg-white rounded-xl shadow-lg p-6 flex flex-col justify-between hover:shadow-xl transition"
    >
      <h2 className="text-2xl font-semibold text-green-900 mb-4">{title}</h2>
...
export default function Home() {
  return (
    <main className="min-h-screen bg-gradient-to-b from-green-100 to-green-300 flex flex-col items-center justify-center px-6 py-12">
      <h1 className="text-5xl font-extrabold text-green-900 mb-8 text-center">
        Welcome to MindCare AI
      </h1>
      <p className="max-w-2xl text-center text-green-800 text-lg mb-12">
        Your AI-powered mental health companion, offering smart emotion detection, chatbot support,
        personalized risk insights, and tailored self-care recommendations.
      </p>
      ...
      <Link
        href="/emotion-detector"
        className="mt-12 px-8 py-4 bg-green-700 text-white rounded-lg font-semibold hover:bg-green-800 transition"
      >
